...
def search_for_date(driver, booking_datetime):
    ...
    # if the desired date is tomorrow, then it is class xo2
    	...
    	driver.find_element(By.XPATH, "//td[@class='xo2 p_AFSelected' and text()='{}']".format(booking_datetime.day)).click()
...
def identify_relevant_courts(driver, court_location):
	...
	relevant_courts = driver.find_elements(By.XPATH, "//span[contains(text(),'{}')]".format(court_location))
...
def book_court(root_url, login_email, login_password, booking_datetime, court_location):
    ...
    for court_link in court_links:
        booking_link, court_name = find_booking_link(driver, court_link, court_location, booking_datetime)
